...
double FiniteFunction::integrate(int Ndiv){
    ...
    double h = (x2 - x1) / Ndiv;
    double y = 0.0;
    y += 0.5 * callFunction(x1);
    for (int i = 1; i < Ndiv; ++i) {
        double x = x1 + i * h;
        y += callFunction(x);  
    }
    y += 0.5 * callFunction(x2);
    return y * h;
}
...
std::vector<double> FiniteFunction::sample_metropolis(int Nsamples, double proposal_width) {
    ...
    std::uniform_real_distribution<double> gen_x(m_RMin, m_RMax);
    std::normal_distribution<double> gen_y(0.0, proposal_width);
    double x = gen_x(gen);

    for (int i = 0; i < Nsamples; i++) {
        double y = x + gen_y(gen);
        if (y < m_RMin || y > m_RMax) {
            continue;
        }
        double fx = callFunction(x);
